...
| Functionally verified hosted guardrails | Security/Release Owners explicitly configure hosted `rai_config`, verify the referenced policy exists and is valid, and run an approved non-destructive expected-block test. Omission disables content safety; invalid policy can fail open despite active status. Reconcile replacement of model policy by agent policy; agent annotate-only is unsupported | Exact effective policy/version/scope, policy existence/validity evidence and observed blocking result for the released deployment; active status/configuration presence alone do not pass | [GOV-06](../controls/README.md#gov-06), [GOV-07](../controls/README.md#gov-07) |
